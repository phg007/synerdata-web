...
import { updateBranch } from "../../services/update-branch";
import {
  formatCEP,
  formatCNPJ,
  formatMobilePhone,
...
import { getBranchById } from "../../services/get-branch-by-id";
...
type UpdateBranchFormData = z.infer<typeof formSchema>;
...
export default function UpdateBranchPage({
  ...
  const { data: branch } = useQuery<BranchObjectResponse>({
  ...
  useEffect(() => {
    if (branch) {
      const [day, month, year] = branch.dataFundacao.split("/");
  ...
  const { mutateAsync: updateBranchFn, isPending } = useMutation({
    ...
    onError: (error: Error) => {
      toast.error("Erro ao atualizar a filial", {
  ...
  const onSubmit = async (data: UpdateBranchFormData) => {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Atualize os dados de uma filial da sua empresa
              </p>
            </div>
          </div>
        </div>

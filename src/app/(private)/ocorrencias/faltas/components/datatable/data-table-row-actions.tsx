...
export function DataTableRowActions<TData>({
  ...
  const { mutateAsync: deleteAbsenceFn } = useMutation({
    mutationFn: deleteAbsence,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["absences", companyId] });
      toast.success("Falta excluída com sucesso.");
    },
    onError: (error: Error) => {
      toast.error("Erro ao excluir a falta.", {
  ...
  async function handleDelete(absenceId: string) {
    try {
      await deleteAbsenceFn({ absenceId });
    } catch (error) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-[160px]">
        ...
        <Link href={`faltas/editar/${row.getValue("id")}`}>
